...
public class Contract {
	...
	public static void setTrade_time_threshold_minutes(
			int tradeTimeThresholdMinutes) {
	...
	// We get the trades at most every hour
	public static int trade_time_threshold_minutes = 1 * 60;
	...
	public Contract(String id, String eventid, String name, String symbol,
			String totalVolume, String ccy, String inRunning, String state,
			String tickSize, String tickValue, String type, Long startDate,
			Long endDate) {
		this.id = id;
	...
	public boolean fetchCSV() {
		String url = "http://data.intrade.com/graphing/jsp/downloadClosingPrice.jsp?contractId="
				+ this.id;
		...
		if (now - this.lastretrievedCSV > time_threshold()) {
			...
			print("Cached:"
					+ DateFormat.getDateTimeInstance().format(
							this.lastretrievedCSV));
		}
		return true;
	}

	public boolean fetchXML() {
		String url = "http://api.intrade.com/jsp/XML/MarketData/ClosingPrice.jsp?conID="
				+ this.id;
		Long now = (new Date()).getTime();
		// If more than it has been some time since last fetch, get it again.
		if (now - this.lastretrievedCSV > time_threshold()) {
	...
	public boolean fetchTrades() {
		...
		String url = "https://api.intrade.com/jsp/XML/TradeData/TimeAndSales.jsp?conID="
				+ this.id;
		Long now = (new Date()).getTime();
		// If more than it has been some time since last fetch, get it again.
	...
	public String toString() {
		return "C:(" + id + ',' + eventid + ',' + symbol + ',' + name + ','
				+ totalVolume + ',' + this.expiryDate + ',' + this.expiryPrice
				+ ',' + ccy + ',' + inRunning + ',' + state + ',' + tickSize
				+ ',' + tickValue + ',' + type + ")";
